...
# Shell session (wrapper) scripts
%define shell_session %{_bindir}/openfoam%{foam_api}
%define shell_session_generic %{_bindir}/openfoam
...
#<with-cgal>
# Horrible, but cgal on Fedora32 copr causes grief and it seems to ignore
# the global
%if %{with cgal}
%if 0%{?fedora}%{?suse_version}
...
# Ownership of /usr/bin/openfoamAPI entrypoint
%ghost %{shell_session}
...
%{prefix}/openfoam
# Ownership of /usr/bin/openfoam entrypoint
%ghost %{shell_session_generic}
